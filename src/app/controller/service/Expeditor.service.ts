...
findAllexpeditors():Observable<Array<ExpeditorVo>>{
  return this.http.get<Array<ExpeditorVo>>('http://localhost:8080/generated/expeditor/');
}
...
if (index > -1) {
   this.expeditorListe.splice(index, 1);
      ...
      this.http.get<ExpeditorVo>('http://localhost:8080/generated/expeditor/title/' + ref).subscribe(
        value => {
        if (value != null) { this.expeditor = value; }
        }
        );
        }
            ...
            value => {
            if (value != null) { this.nationalitys = value; }
            }
            );
            }
